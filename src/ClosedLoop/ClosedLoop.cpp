...
GCodeResult ClosedLoop::ProcessM569Point1(CanMessageGenericParser& parser, const StringRef& reply) noexcept
{
	...
	// Set the new closed loop parameters
	{
		...
		if (seenPid)
		{
			...
			if (seenI)
			{
				splitIntegral = seenKiStandstill;
				KiStandstill = seenKiStandstill ? tempKiStandstill : tempKi;
			}
			if (seenD)
			{
				velocityDerivative = seenKdStandstill;
				KdStandstill = seenKdStandstill ? tempKdStandstill : tempKd;
			}
			Kf = tempKf;
			Kv = tempKv;
			Ka = tempKa;
...
void ClosedLoop::InstanceControlLoop(StepTimer::Ticks now, StepTimer::Ticks timeElapsed) noexcept
{
	...
	if (encoder != nullptr && encoder->TakeReading())
	{
		...
		if (currentMode != ClosedLoopMode::closed || tuning != 0 || tuningError != 0)
		{
			frictionFeedforward.Reset();
		}
...
inline float ClosedLoop::ControlMotorCurrents(StepTimer::Ticks ticksSinceLastCall) noexcept
{
	...
	else
	{
		...
		if (currentMode == ClosedLoopMode::closed)
		{
			...
			const float motionBlend = MotionTransition::Calculate(mParams.speed, mParams.acceleration,
				ticksSinceLastCall, SpeedFilterSize);
			if (velocityDerivative)
			{
				const float effectiveKd = KdStandstill + motionBlend * (Kd - KdStandstill);
				PIDDTerm = constrain<float>(effectiveKd * (mParams.speed - speedFilter.GetDerivative())
					* StepTimer::StepClockRate, -256.0, 256.0);
			}
			PIDFTerm = frictionFeedforward.Update(mParams.speed, Kf, motionBlend, SpeedFilterSize);
			if (!splitIntegral)
			{
			...
			if (splitIntegral)
			{
				const float effectiveKi = KiStandstill + motionBlend * (Ki - KiStandstill);
			...
			PIDControlSignal = (Kf == 0.0f)
				? constrain<float>(PIDPTerm + PIDITerm + PIDDTerm + PIDVTerm + PIDATerm, -256.0, 256.0)
				: constrain<float>(PIDPTerm + PIDITerm + PIDDTerm + PIDFTerm + PIDVTerm + PIDATerm, -256.0, 256.0);
